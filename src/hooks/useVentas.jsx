...
export const useVentas = () => {
  ...
  const fetchVentasData = useCallback(async () => {
    ...
    try {
      ...
      console.log(resVent.data.ventas)
  ...
  const calculos = useMemo(() => {
    ...
    // 2. Cálculo Monetario
    const subtotalBruto = cantidadNeta * prec

    // Lo que el cliente abona físicamente hoy (si es crédito usa el input, si es contado es el saldo tras anticipo)
    const abonoHoy = formData.esCredito
      ? parseFloat(formData.abonoManual) || 0
      : subtotalBruto - anticipoRecibido

    const saldoFinal = subtotalBruto - anticipoRecibido - abonoHoy
    ...
    return {
      ...
      abonado: abonoHoy, // Este valor va al registro de caja
